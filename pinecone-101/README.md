...
5. Run `procedural_create_load.py` to embed and upsert data
6. Run `procedural_vector_search.py` to perform a vector search
...
Run the main `procedural_create_load.py` script to embed the data from `demo_doctor_notes.jsonl` and UPSERT data to Pinecone:
```bash
